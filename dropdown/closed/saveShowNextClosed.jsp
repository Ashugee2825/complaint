...
Closed closed = (Closed)request.getAttribute("closed");
...
<input required type="text" name="code" value="<%=closed.getCode()%>" size="30">
</td>

<td>
VALUE<font color="red">*</font>:<br/>
<input required type="text" name="value" value="<%=closed.getValue()%>" size="30">
</td>
...
<input type="hidden" name="page" value= "saveShowNextClosed">
<input type="hidden" name="pageNo" value= "<%=pageNo%>">
<input type="hidden" name="limit" value= "<%=limit%>">
<input type="hidden" name="id" value= "<%=closed.getId()%>">
</form>

<form action="closedCntrl"  id="myform1" name="myform1" accept-charset="UTF-8" >
<input type="hidden" name="page" value= "saveShowNextClosed">
<input type="hidden" name="pageNo" value= "<%=pageNo%>">
<input type="hidden" name="limit" value= "<%=limit%>">
<input type="hidden" name="id" value= "<%=closed.getId()%>">
...
<% dropDownDBService.closeConnection(); %>
    ...
    <% Iterator<Closed> it1= closedList.iterator(); %>
    
<%if(pageNum!=0){ %>   	
	<div align="left">	
	<font color="blue" size="3">	
	<%if(pageNum==1){%>|&lt;&nbsp;&nbsp;&nbsp;&lt;&lt;<%} else { %><a href="closedCntrl?page=closedDashboard&opr=showAll&pageNo=<%=1%>&limit=<%=limit%>">|&lt;</a>&nbsp;&nbsp;&nbsp;<a href="closedCntrl?page=closedDashboard&opr=showAll&pageNo=<%=pageNum-1%>&limit=<%=limit%>">&lt;&lt;</a><%}%>	
	&nbsp;&nbsp;&nbsp;Page: <%=pageNum%>/<%=totalPages%>	
	<%if(pageNum==totalPages){%>&nbsp;&nbsp;&nbsp;&gt;&gt;&nbsp;&nbsp;&nbsp;&gt;|<%} else { %>&nbsp;&nbsp;&nbsp;<a href="closedCntrl?page=closedDashboard&opr=showAll&pageNo=<%=pageNo+1%>&limit=<%=limit%>">&gt;&gt;</a>&nbsp;&nbsp;&nbsp;<a href="closedCntrl?page=closedDashboard&opr=showAll&pageNo=<%=totalPages%>&limit=<%=limit%>">&gt;|</a><%}%>	
	</font>	
	</div>    	
	<%} %>	
...
<% while(it1.hasNext()){
	Closed closed1 = (Closed)it1.next();
	%>
<tr>
<td><%=count++%></td>
<td><%=closed1.getId()%></td>
<td><%=closed1.getCode()%></td>
<td><%=closed1.getValue()%></td>
<td><a href="closedCntrl?page=saveShowNextClosed&opr=edit&id=<%=closed1.getId()%>&pageNo=<%=pageNo%>&limit=<%=limit%>"><button type="button" class="btn">Edit</button></a></td>
<td><a href="closedCntrl?page=saveShowNextClosed&opr=delete&id=<%=closed1.getId()%>&pageNo=<%=pageNo%>&limit=<%=limit%>"><button type="button" class="btn1">Delete</button></a></td>
</tr>
	<%} %>
</tbody>
	</table>    </div>
