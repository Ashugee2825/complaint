...
Cancelled cancelled = (Cancelled)request.getAttribute("cancelled");
...
<input required type="text" name="code" value="<%=cancelled.getCode()%>" size="30">
</td>

<td>
VALUE<font color="red">*</font>:<br/>
<input required type="text" name="value" value="<%=cancelled.getValue()%>" size="30">
</td>
...
<input type="hidden" name="page" value= "saveShowNextCancelled">
<input type="hidden" name="pageNo" value= "<%=pageNo%>">
<input type="hidden" name="limit" value= "<%=limit%>">
<input type="hidden" name="id" value= "<%=cancelled.getId()%>">
</form>

<form action="cancelledCntrl"  id="myform1" name="myform1" accept-charset="UTF-8" >
<input type="hidden" name="page" value= "saveShowNextCancelled">
<input type="hidden" name="pageNo" value= "<%=pageNo%>">
<input type="hidden" name="limit" value= "<%=limit%>">
<input type="hidden" name="id" value= "<%=cancelled.getId()%>">
...
<% dropDownDBService.closeConnection(); %>
    ...
    <% Iterator<Cancelled> it1= cancelledList.iterator(); %>
    
<%if(pageNum!=0){ %>   	
	<div align="left">	
	<font color="blue" size="3">	
	<%if(pageNum==1){%>|&lt;&nbsp;&nbsp;&nbsp;&lt;&lt;<%} else { %><a href="cancelledCntrl?page=cancelledDashboard&opr=showAll&pageNo=<%=1%>&limit=<%=limit%>">|&lt;</a>&nbsp;&nbsp;&nbsp;<a href="cancelledCntrl?page=cancelledDashboard&opr=showAll&pageNo=<%=pageNum-1%>&limit=<%=limit%>">&lt;&lt;</a><%}%>	
	&nbsp;&nbsp;&nbsp;Page: <%=pageNum%>/<%=totalPages%>	
	<%if(pageNum==totalPages){%>&nbsp;&nbsp;&nbsp;&gt;&gt;&nbsp;&nbsp;&nbsp;&gt;|<%} else { %>&nbsp;&nbsp;&nbsp;<a href="cancelledCntrl?page=cancelledDashboard&opr=showAll&pageNo=<%=pageNo+1%>&limit=<%=limit%>">&gt;&gt;</a>&nbsp;&nbsp;&nbsp;<a href="cancelledCntrl?page=cancelledDashboard&opr=showAll&pageNo=<%=totalPages%>&limit=<%=limit%>">&gt;|</a><%}%>	
	</font>	
	</div>    	
	<%} %>	
...
<% while(it1.hasNext()){
	Cancelled cancelled1 = (Cancelled)it1.next();
	%>
<tr>
<td><%=count++%></td>
<td><%=cancelled1.getId()%></td>
<td><%=cancelled1.getCode()%></td>
<td><%=cancelled1.getValue()%></td>
<td><a href="cancelledCntrl?page=saveShowNextCancelled&opr=edit&id=<%=cancelled1.getId()%>&pageNo=<%=pageNo%>&limit=<%=limit%>"><button type="button" class="btn">Edit</button></a></td>
<td><a href="cancelledCntrl?page=saveShowNextCancelled&opr=delete&id=<%=cancelled1.getId()%>&pageNo=<%=pageNo%>&limit=<%=limit%>"><button type="button" class="btn1">Delete</button></a></td>
</tr>
	<%} %>
</tbody>
	</table>    </div>
