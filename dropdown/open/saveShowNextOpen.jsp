...
Open open = (Open)request.getAttribute("open");
...
<input required type="text" name="code" value="<%=open.getCode()%>" size="30">
</td>

<td>
VALUE<font color="red">*</font>:<br/>
<input required type="text" name="value" value="<%=open.getValue()%>" size="30">
</td>
...
<input type="hidden" name="page" value= "saveShowNextOpen">
<input type="hidden" name="pageNo" value= "<%=pageNo%>">
<input type="hidden" name="limit" value= "<%=limit%>">
<input type="hidden" name="id" value= "<%=open.getId()%>">
</form>

<form action="openCntrl"  id="myform1" name="myform1" accept-charset="UTF-8" >
<input type="hidden" name="page" value= "saveShowNextOpen">
<input type="hidden" name="pageNo" value= "<%=pageNo%>">
<input type="hidden" name="limit" value= "<%=limit%>">
<input type="hidden" name="id" value= "<%=open.getId()%>">
...
<% dropDownDBService.closeConnection(); %>
    ...
    <% Iterator<Open> it1= openList.iterator(); %>
    
<%if(pageNum!=0){ %>   	
	<div align="left">	
	<font color="blue" size="3">	
	<%if(pageNum==1){%>|&lt;&nbsp;&nbsp;&nbsp;&lt;&lt;<%} else { %><a href="openCntrl?page=openDashboard&opr=showAll&pageNo=<%=1%>&limit=<%=limit%>">|&lt;</a>&nbsp;&nbsp;&nbsp;<a href="openCntrl?page=openDashboard&opr=showAll&pageNo=<%=pageNum-1%>&limit=<%=limit%>">&lt;&lt;</a><%}%>	
	&nbsp;&nbsp;&nbsp;Page: <%=pageNum%>/<%=totalPages%>	
	<%if(pageNum==totalPages){%>&nbsp;&nbsp;&nbsp;&gt;&gt;&nbsp;&nbsp;&nbsp;&gt;|<%} else { %>&nbsp;&nbsp;&nbsp;<a href="openCntrl?page=openDashboard&opr=showAll&pageNo=<%=pageNo+1%>&limit=<%=limit%>">&gt;&gt;</a>&nbsp;&nbsp;&nbsp;<a href="openCntrl?page=openDashboard&opr=showAll&pageNo=<%=totalPages%>&limit=<%=limit%>">&gt;|</a><%}%>	
	</font>	
	</div>    	
	<%} %>	
...
<% while(it1.hasNext()){
	Open open1 = (Open)it1.next();
	%>
<tr>
<td><%=count++%></td>
<td><%=open1.getId()%></td>
<td><%=open1.getCode()%></td>
<td><%=open1.getValue()%></td>
<td><a href="openCntrl?page=saveShowNextOpen&opr=edit&id=<%=open1.getId()%>&pageNo=<%=pageNo%>&limit=<%=limit%>"><button type="button" class="btn">Edit</button></a></td>
<td><a href="openCntrl?page=saveShowNextOpen&opr=delete&id=<%=open1.getId()%>&pageNo=<%=pageNo%>&limit=<%=limit%>"><button type="button" class="btn1">Delete</button></a></td>
</tr>
	<%} %>
</tbody>
	</table>    </div>
